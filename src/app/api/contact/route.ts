...
function buildMessage(inquiry: Inquiry, id: string) {
  const subject = `Platinum inquiry — ${inquiry.company || inquiry.name}`;
  const text = [
    "New inquiry from the Platinum website.",
    ...
    "",
    `Scale: ${inquiry.scale || "—"}`,
    `Class: ${inquiry.gpuClass || "—"}`,
    `Timing: ${inquiry.timing || "—"}`,
    `Boundary: ${inquiry.boundary || "—"}`,
  ].join("\n");
...
export async function POST(request: Request) {
  ...
  const inquiry: Inquiry = {
    ...
    scale: readString(body.scale, 400),
    gpuClass: readString(body.class, 200),
    timing: readString(body.timing, 400),
    boundary: readString(body.boundary, 2000),
  };
    ...
    return NextResponse.json(
      {
        ...
        error: configured
          ...
          : "Inquiry email is not configured on the server yet.",
        requestId: id,
        mailto: Boolean(site.email),
      },
